...
int main(int argc, char **argv)
{
	...
	while (fgets(line, 200, stdin) != NULL)
	{
		...
		{
			int res = 0;
			int unknown[d];				// unknown
			int count = 0;				// count of the unknown bits
			for (int i = 0; i < d; i++) // iterating over d
			{
				if (!q[cv[i].attr].known)						  // check conditions
					unknown[count++] = i;						  // putting vale of i
				else if (bitIsSet(q[cv[i].attr].hash, cv[i].bit)) // otherwise checking
					res = setBit(res, i);						  // result
			}
			int power = 1;
			int temp = count;
			while (temp != 0) // while temp is not 0
			{
				power *= 2; // power
				temp--;		// decrement
			}
			printf("Pages: ");
			for (int i = 0; i < power; i++) // iterating over the truth table
			{
				int out = res; // temp out
				for (int j = 0; j < count; j++)
					if (bitIsSet(i, j))
						out = setBit(out, unknown[j]); // out
				printf("%d ", out);					   // print pages
			}
			printf("\n");
		}
		printf("query> ");
